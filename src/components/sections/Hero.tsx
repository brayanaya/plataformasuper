...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative bg-black text-white min-h-[calc(100vh-88px)] px-6 flex flex-col items-center justify-center text-center gap-6 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #dc2626 0%, transparent 50%), radial-gradient(circle at 80% 20%, #fbbf24 0%, transparent 40%)' }} />
      <div className={'relative z-10 flex flex-col items-center gap-6 transition-all duration-1000 ' + (visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10')}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold leading-tight">
            Tu mercado de<br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-200">confianza</span>
          </h1>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-2 w-full max-w-sm sm:max-w-none justify-center">
          <button onClick={() => document.getElementById('catalogo')?.scrollIntoView({ behavior: 'smooth' })} className="bg-gradient-to-r from-yellow-400 to-yellow-300 text-black font-extrabold px-8 py-3 rounded-full hover:from-yellow-300 hover:to-yellow-200 transition shadow-lg shadow-yellow-400/30">Ver Catalogo</button>

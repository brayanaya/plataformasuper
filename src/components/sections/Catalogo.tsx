      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <div ref={scrollRef} className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            {categorias.map((c) => (
              <button key={c.id} onClick={() => handleCat(c.id)} className={categoriaActiva === c.id ? 'flex-shrink-0 px-3 py-1 rounded-full text-xs font-bold bg-red-700 text-white shadow' : 'flex-shrink-0 px-3 py-1 rounded-full border border-gray-200 bg-white text-gray-600 text-xs font-medium hover:border-red-700 hover:text-red-700 transition'}>{c.nombre}</button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="bg-white rounded-2xl overflow-hidden animate-pulse">
                ...
                <div className="p-3 space-y-2"><div className="h-2.5 bg-gray-200 rounded w-3/4" /><div className="h-2.5 bg-gray-200 rounded w-1/2" /></div>
              </div>
            ))}
          </div>
          ...
          <>
            ...
            {hayMas && (
              <div className="text-center mt-8">
                ...
                <button onClick={() => setPagina(p => p + 1)} className="bg-white border border-gray-200 text-gray-700 font-bold px-8 py-2.5 rounded-full hover:border-red-700 hover:text-red-700 transition text-sm shadow-sm">
                  Ver mas productos
                </button>
              </div>
            )}
            {!hayMas && productosFiltrados.length > POR_PAGINA && (
              <p className="text-center text-gray-400 text-xs mt-6">Mostrando todos los productos</p>
            )}
          </>
        )}
      </div>

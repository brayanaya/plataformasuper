...
export default function Catalogo() {
  ...
  return (
    <section id="catalogo" className="bg-gray-200 py-12 px-4 md:px-6">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-2xl md:text-4xl font-extrabold text-red-700">Catalogo</h2>
          <p className="text-gray-600 mt-2 text-xs md:text-base">Encuentra todo lo que necesitas</p>
          <div className="w-12 h-1 bg-yellow-400 mx-auto mt-3 rounded-full" />
        </div>
        <div className="flex items-center gap-2 mb-8">
          <button onClick={scrollLeft} className="flex-shrink-0 w-8 h-8 rounded-full bg-white shadow border border-gray-300 flex items-center justify-center text-red-700 font-bold hover:bg-red-50 transition">&#8249;</button>
          <div ref={scrollRef} className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            {categorias.map((c) => (
              <button key={c.id} onClick={() => setCategoriaActiva(c.id)} className={categoriaActiva === c.id ? 'flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-bold bg-red-700 text-white shadow' : 'flex-shrink-0 px-4 py-1.5 rounded-full border border-gray-300 bg-white text-gray-600 text-xs font-medium hover:border-red-700 hover:text-red-700 transition'}>{c.nombre}</button>
            ))}
          </div>
          <button onClick={scrollRight} className="flex-shrink-0 w-8 h-8 rounded-full bg-white shadow border border-gray-300 flex items-center justify-center text-red-700 font-bold hover:bg-red-50 transition">&#8250;</button>
        </div>
        {loading ? (
          <div className="text-center text-gray-500 py-20">Cargando productos...</div>
        ) : productosFiltrados.length === 0 ? (
          <div className="text-center text-gray-500 py-20">No hay productos disponibles</div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
            {productosFiltrados.map((p) => (
              <div key={p.id} className="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-lg transition group">
                {p.imagen_url ? (
                ...
                <div className="p-3">
                  ...
                  <button onClick={() => handleAgregar(p)} className={agregados[p.id] ? 'mt-2 w-full font-bold text-xs py-2 rounded-full bg-green-500 text-white transition-all duration-300' : 'mt-2 w-full font-bold text-xs py-2 rounded-full bg-yellow-400 text-black hover:bg-yellow-300 transition-all duration-300'}>

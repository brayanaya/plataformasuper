    ...
    <section id="catalogo" className="bg-white py-12 px-4 md:px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-8">
          <button onClick={scrollLeft} className="flex-shrink-0 w-8 h-8 rounded-full bg-white shadow border border-gray-200 flex items-center justify-center text-red-700 font-bold hover:bg-red-50 transition">&#8249;</button>
          <div ref={scrollRef} className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            {categorias.map((c) => (
              <button key={c.id} onClick={() => setCategoriaActiva(c.id)} className={categoriaActiva === c.id ? 'flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-bold bg-red-700 text-white' : 'flex-shrink-0 px-4 py-1.5 rounded-full border border-gray-300 text-gray-600 text-xs font-medium hover:border-red-700 hover:text-red-700 transition'}>{c.nombre}</button>
            ))}
          </div>
          <button onClick={scrollRight} className="flex-shrink-0 w-8 h-8 rounded-full bg-white shadow border border-gray-200 flex items-center justify-center text-red-700 font-bold hover:bg-red-50 transition">&#8250;</button>
        </div>
        {loading ? (

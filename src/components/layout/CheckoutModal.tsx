...
export default function CheckoutModal({ open, onClose }: Props) {
  ...
  const handlePedido = async () => {
    ...
    const listaProductos = items.map((i) => i.nombre + ' x' + i.cantidad + ' ($' + (i.precio * i.cantidad).toLocaleString('es-CO') + ')').join(', ')
    const mensajeWA = 'Hola! Soy ' + nombre + '. Pedido por $' + totalVal.toLocaleString('es-CO') + '. Productos: ' + listaProductos + '. Direccion: ' + direccion + (barrio ? ', Barrio ' + barrio : '') + '. Tel: ' + telefono
        ...
        {exito ? (
          ...
          <>
            <div className="bg-red-700 px-8 py-5 text-center">
              ...
              <p className="text-red-200 text-sm mt-1">Total: {'$'}{total().toLocaleString('es-CO')}</p>
            </div>

...
export default function CheckoutModal({ open, onClose }: Props) {
  ...
  const handlePedido = async () => {
    ...
    const mensajeWA = "Hola! Soy " + nombre + ". Acabo de realizar un pedido por $" + total().toLocaleString() + ". Productos: " + listaProductos + ". Direccion: " + direccion + (barrio ? ", Barrio " + barrio : "") + ". Tel: " + telefono
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        {exito ? (
          <div className="p-10 text-center flex flex-col gap-4">
            ...
            <p className="text-gray-600 text-sm leading-relaxed">Tu pedido fue registrado y te redirigimos a WhatsApp para confirmarlo con nosotros.</p>
            <button onClick={() => { setExito(false); onClose(); setNombre(""); setTelefono(""); setDireccion(""); setBarrio("") }} className="bg-red-700 text-white font-bold py-3 rounded-xl hover:bg-red-600 transition">
              Cerrar
            </button>
          </div>
        ) : (
          <>
            <div className="bg-red-700 px-8 py-5 text-center">
              ...
              <p className="text-red-200 text-sm mt-1">Total: ${total().toLocaleString()}</p>
            </div>

      ...
      <nav className="bg-red-700 text-white px-8 py-4 shadow-lg sticky top-0 z-20">
        <div className="flex items-center justify-between">
          ...
          <ul className="hidden md:flex gap-3 text-sm font-semibold items-center">
            ...
            {usuario ? (
              <>
                {esAdmin && (<li><button onClick={() => navigate('/admin/dashboard')} className="bg-yellow-400 text-black px-5 py-2 rounded-full font-bold hover:bg-yellow-300 transition">Panel Admin</button></li>)}
            ...
            <li>
              <button onClick={() => setCartOpen(true)} className="relative bg-yellow-400 text-black px-4 py-2 rounded-full font-bold hover:bg-yellow-300 transition">
                {'🛒'}
                {totalItems > 0 && (<span className="absolute -top-2 -right-2 bg-black text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{totalItems}</span>)}
              </button>
            </li>
          </ul>
          <div className="flex items-center gap-3 md:hidden">
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="text-white text-2xl font-bold">{menuOpen ? 'x' : '='}</button>
          </div>
        </div>
        {menuOpen && (
